...
LOG_FILE = os.path.join(LOG_DIR, 'fetch_lof_szse.log')
...
def setup_logger(name='fetch_lof_szse'):
...
def fetch_all_lof_szse(logger):
    """
    从东方财富searchapi获取深市LOF数据
    注：searchapi返回的1xx代码对应深交所LOF，与深交所数据同步
    """
    import requests
    
    logger.info("开始获取深市LOF数据")
...
def merge_data(old_data, new_data, logger):
    """合并数据，保留沪市LOF，替换深市LOF"""
    # 保留非SZLOF
    sh_lofs = [f for f in old_data if f.get('type') != 'SZLOF']
    # 新的SZLOF
    merged = sh_lofs + new_data
    logger.info(f"合并完成: 沪市SHLOF={len(sh_lofs)}, 深市SZLOF={len(new_data)}, 总计={len(merged)}")
    return merged
...
def main():
    ...
    # 获取数据
    new_data = fetch_all_lof_szse(logger)
    if not new_data:
        logger.error("获取数据失败")
        return
